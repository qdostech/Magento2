...
class CsvImportHandler
{
	...
	public function readCsvFile($PfilePath, $params,$client,$logMsg){
		...
		foreach($productData as $product){	
			...
			if(isset($product['image']) && $product['image'] !== ''){
				$product['image']= $baseurl."media/import".$product['image'];
			}
			if(isset($product['small_image']) && $product['small_image'] !== ''){
				$product['small_image']= $baseurl."media/import".$product['small_image'];
			}
			if(isset($product['thumbnail']) && $product['thumbnail'] !== ''){
				$product['thumbnail']= $baseurl."media/import".$product['thumbnail'];
			}
			// print_r($product);die;
			/* change for image full path end */
			// $arr = array('3445');
			//$arr = array('631','661','1379','1385','1418','1425','2871','2894','3190','3193','3197','3200','3201','3433','3453','3455','3518','3569','3585','3616','3621','3643','3645','3646','3648','3649','3650','3652','3653','3654','3655','3656');
			// if(!in_array($product['sku'], $arr)){
			// 	continue;
			// }
			$client->setLog("product sku--".$product['sku'],null,"syncProduct-".date('Ymd').".log");
			$logMsg[] = "product sku-".$product['sku'];
			$logMsg = $this->requiredDataForSaveProduct($product, $params, $logMsg);
			$client->setLog($i." product sku-- end here",null,"syncProduct-".date('Ymd').".log");
			$i++;
		}
		return $logMsg;
	}
	...
	public function ProductData($product, $params){
		...
		*/
		...
		// if(empty($product['url_key'])) { $defaultProductData['url_key'] = $product['name'].'-'.$product['sku']; } else { $defaultProductData['url_key'] = $product['url_key'].'-'.$product['sku']; }
		// if(empty($product['url_key'])) { $defaultProductData['url_key'] = str_replace(' ','-',rtrim(strtolower($product['meta_title']))); } else { $defaultProductData['url_key'] = $product['url_key']; }
	...
	public function ProductAttributeData($product){
		...
		// if(empty($product['url_key'])) { $defaultProductData['url_key'] = $product['name'].'-'.$product['sku']; } else { $defaultProductData['url_key'] = $product['url_key']; }
		// $defaultAttributeData['url_key'] = (empty($product['url_key'])) ? $product['name'].'-'.$product['sku'] : $product['url_key'].'-'.$product['sku'];
		// $defaultAttributeData['url_key'] = (empty($product['url_key'])) ? str_replace(' ','-',trim($product['meta_title'])) : $product['url_key'];
		// $defaultAttributeData['url_path'] = (empty($product['url_path'])) ? str_replace(' ','-',trim($product['meta_title'])) : $product['url_path'];
	...
	public function ProductSupperAttribute($product, $params){
		...
		$defaultSupperAttributeData = array(
			...
			'tier_prices'                 => (isset($product['tier_prices'])) ? $this->TierPricedata($product['tier_prices'], $product['sku'], $params) : '',
	...
	public function TierPricedata($TPData, $product_sku, $params){
