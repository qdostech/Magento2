...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function syncAttribute()
    {
        ...
        $this->_syncattributelog->setStartTime($start_time)
            ...
            ->save();
        /*Log code*/
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/syncAttribute.log');
    ...
    public function syncGrape()
    {
        ...
        try {
            ...
            $grapeAttributeDetails = $resultClient->GetAttributesDetailsCSV(array('store_url' => $store_url, 'attribute_code' => 'grape'));
            //echo '<pre>';print_r($grapeAttributeDetails);exit;
            $collectionData = $grapeAttributeDetails->GetAttributesDetailsCSVResult;
            ...
            /*Log code*/
            $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/wineryLog.log');
            $logger = new \Zend\Log\Logger();
            $logger->addWriter($writer);
            ...
            foreach ($collectionData as $items) {
                foreach ($items as $itemes) {
                    ...
                    foreach ($itemes as $key => $item) {
                        ...
                        $additem[strtolower($key)] = $item;
                    }
                    $grape_model = $objectManager->create('\Neo\Winery\Model\Syncgrapes');
                    $option_exist = $grape_model->load($check, 'ATTRIBUTE_VALUE');
//
                    if (!empty($option_exist->getData())) {
                        $option_exist->setData($additem);
                        $option_exist->save();
                    } else {
                        $grape_model->setData($additem);
                        $grape_model->save();
                    }
                }
            }
            ...
            $client->setLog("Sync Grape failed" . print_r($e->getMessage(), true), null, "sync_grape_options_-" . date('Ymd') . ".log");
        }
        ...
        try {
            ...
            if (file_exists($productcsv) && (!is_writable($productcsv) || !is_writable($productcsvother))) {
                ...
                foreach ($collection as $item) {
                    ...
                    $client->setLog('After if else '.$item['sku'].' - '.$item['url_key'], null, $logFileName);
...
$client->setLog($item['type'], null, $logFileName);
