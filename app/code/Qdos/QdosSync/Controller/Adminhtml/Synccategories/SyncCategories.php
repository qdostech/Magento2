...
namespace Qdos\QdosSync\Controller\Adminhtml\Synccategories;
set_time_limit(0);
...
class SyncCategories extends \Magento\Backend\App\Action
{
    /**
    ...
    public function execute()
    {
        ...
        $syncCatStatus = $this->_scopeConfig->getValue('qdosConfig/permissions/manual_category_sync', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        ...
        if ($syncCatStatus) {
            ...
            if (strtolower($cronStatus) == 'running') {
                ...
                $result = $objectManager->create('Qdos\QdosSync\Helper\Data')->syncCategory($_SERVER['REMOTE_ADDR'], $storeId);

                if ($result) {
                    $logMsg = 'Categories in store were synchronized success.';
                    $this->_logger->info($logMsg);
                    $this->messageManager->addSuccess(__($logMsg));
                } else {
                    $logMsg = 'Can not synchronize some categories in this store.';
                    $this->_logger->info($logMsg);
                    $this->messageManager->addError(__($logMsg));
                }
    ...
    /*public function execute()
    {
        $resultRedirect = $this->resultRedirectFactory->create();
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $result = $objectManager->create('Qdos\QdosSync\Helper\Data')->syncCategory($_SERVER['REMOTE_ADDR']);
        if ($result) {
            $message = __('Categories in store were synchronized success.');
            $this->messageManager->addSuccess($message);
        } else {
            $this->messageManager->addError(__('Can not synchronize some categories in this store.'));
        }
        $resultRedirect->setPath('qdossync/synccategories/index/');
