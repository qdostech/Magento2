    ...
    $this->_logger->info(__METHOD__);
        ...
        $logFileName = "import-".date('Ymd').".log";
                ...
                $this->_log->setStartTime($start_time)
                 ...
                 try{
                      $result = $resultClient->GetAttributesResult;
                      
                      if (is_object($result) && isset($result->Attributes)) {
                        $collection = $this->convertObjToArray($result->Attributes);
...
public function addAttributeToAllAttributeSets(string $attributeCode, string $attributeGroupCode)
    ...
    **/
    ...
    public function ProductsSync($filePath, $syncPermissions) {
      $this->_logger->info(__METHOD__);
      $base = $this->directory_list->getPath('lib_internal');
      $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
      $lib_file = $base.'/Test.php';
  ...
  /**
   * Gets category collection to be synch in magento.
   *
   * @author Pradeep Sanku <pradeep.sanku@wwindia.com>
  */
  public function syncCategory(){
    ...
    try {
      ...
      $this->_log->setStartTime($start_time)
        ...
        ->setStatus(\Qdos\QdosSync\Model\Activity::LOG_PENDING)
        ->setIpAddress($ipAddress)
        ->setActivityType('SYNC CATEGORIES')
        ->save();
      ...
      if (count($this->_lostDataArr) > 0){
        ...
        foreach ($this->_lostDataArr as $data){
          $error[] = '<b style="color:red">Error: '.$data['name'].'('.$data['id'].')</b>';
        }
      }
  ...
  /**
   * Passes one by one category collection to import in magento
   *
   * @author Pradeep Sanku <pradeep.sanku@wwindia.com>
  */
      ...
      foreach ($collectionData as $items) {
        ...
        if (!$item['PAR_ROW_ID'] || $item['PAR_ROW_ID'] == 0) {
          if ($storeId) {
            ...
            $item['PAR_ROW_ID'] = \Magento\Catalog\Model\Category::TREE_ROOT_ID;
          }
        }
        ...
        if ($parentCategory->getId()) {
          ...
          $client->setLog('item lost: '.$item,null,"syncCategory-".date('Ymd').".log");
          $logMsg[] = 'item lost: '.$item;
        }
      }
    }
    return $logMsg;
  ...
  /**
   * import single single category in magento
   *
   * @author Pradeep Sanku <pradeep.sanku@wwindia.com>
  */
  ...
  */
  protected function _deleteAllCategories($storeId,$importData,$client){
    ...
    try {
      ...
      // $client->setLog($rootId,null,'root.log');
      // $client->setLog(\Magento\Catalog\Model\Category::TREE_ROOT_ID,null,'TR.log');
      foreach ($collection as $category) {
        // $client->setLog($category->getId(),null,'collection.log');
        if (!in_array($category->getId(),$importData) &&
            ...
            $client->setLog($category->getId(),null,'GG.log');
              $category->delete();
              $client->setLog('_deleteAllCategories',null,$logFileName);
  ...
  public function addSubcategories($parentId,$catId,$catName,$allCatArray,$currentCat){
    ...
    //$filePath['simpleProduct'] =  $this->directory_list->getPath('var'). '/import/import_products_simple_single.csv'; //$productcsv and $filePath should be same
    // $filePath['simpleProduct'] =  $this->directory_list->getPath('var'). '/import/import_products_config_new.csv';

    $filePath['simpleProduct'] =  $this->directory_list->getPath('var'). '/import/import_products_bundle.csv';
    $filePath['otherProduct'] =  $this->directory_list->getPath('var'). '/import/import_example_products_config_new.csv';
    ...
    try {
      ...
      */
      ...
      $productcsv = $this->directory_list->getPath('var'). '/import/import_products_bundle.csv';
      chmod($productcsv, 0777);
      $productcsvother = $this->directory_list->getPath('var'). '/import/import_example_products_config_new.csv';
      ...
      if (file_exists($productcsv) && (!is_writable($productcsv) || !is_writable($productcsvother))) 
      {
        ...
        if ($resultClient->outErrorMsg && strlen($resultClient->outErrorMsg) > 0){
          ...
          if (is_object($result) && isset($result->ProductCSV)) {
            ...
            //echo "<pre> print product csv object";print_r($objCollection);exit;
          }
        }
      ...
      $headerflag = 0;
      $file = fopen($productcsv,"w");
      $fileother = fopen($productcsvother,"w");
      //chmod($file, 0777);
      $csvarray = array();
      $csvotherarray = array();
      $arrUrlKey = array();
      ...
      foreach ($collection as $item) {
        ...
        if (isset($item['url_key'])) {
          /*echo $item['url_key'];
            exit; - o/p : P/S-Slipper-Canvas-White/6*/
            ...
            if(isset($item['bundle_options'])){
                    ...
                    if (strtolower($item['type']) == 'configurable') {  
                        ...
                        foreach ($associatedProducts as $akey => $productSku) {
                            ...
                            if (array_key_exists($productSku, $arrProductData)) { 
                                ...
                                /*echo $fixedPriceForConfig;
                                exit;*/

                                //$fixedPriceForConfig = Mage::getStoreConfig('qdos_sync_config/permissions/fixed_price_config_child');
                    ...
                    $enableTierPriceSync =$objectManager->get('Magento\Framework\App\Config\ScopeConfigInterface')->getValue('qdosConfig/permissions/tier_price_sync'); 
                        if ($enableTierPriceSync) {
                            if (isset($item['tier_price']) || $item['tier_price'] != ''){
                ...
                if($objectManager->get('Magento\Framework\App\Config\ScopeConfigInterface')->getValue('qdosConfig/permissions/product_image_import_sync')){
                    ...
                    if ($storeId == 0) {
                        ...
                        fputcsv($fileother, array_keys($finalarray[0]));
                        $headerflag = 1;
                ...
                //Mage::log($cache_response_msg, 1, $logFileName, true);

                $returnMsgs = $this->ProductsSync($filePath['otherProduct'], $syncPermissions);
